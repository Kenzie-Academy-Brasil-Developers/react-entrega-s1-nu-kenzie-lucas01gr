...
function Card({ transaction, del }) {
  return (
    <div>
      <div className="colorida">
        <div className="descricaoEvalor">
          <div className="divDescricao">
            <p className="descricao">{transaction.description}</p>
          </div>
          <p className="valor">R$ {transaction.value}</p>
          {/* <img onClick={() => del(transaction.description)}>img </img> */}
          <svg
            className="imgTrash"
            onClick={() => del(transaction.description)}
            width="9"
            height="10"
            viewBox="0 0 9 10"
            fill="none"
